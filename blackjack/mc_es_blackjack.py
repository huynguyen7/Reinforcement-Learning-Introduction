...
def mc_exploring_starts(num_episodes=500000, gamma=1.0):  # Monte Carlo Exploring Starts
    ...
    for episode in tqdm(range(num_episodes)):
        ...
        for (player_sum, dealer_upcard, has_usable_ace) in reversed(player_history):
            ...
            state_action = ((player_sum, dealer_upcard, has_usable_ace), random_action)
            if state_action not in visited_states:
                visited_states.add(state_action)
                if has_usable_ace:
                    usable_ace_returns[player_sum-12, dealer_upcard-1, random_action] = gamma*usable_ace_returns[player_sum-12, dealer_upcard-1, random_action] + reward
                    usable_ace_N[player_sum-12, dealer_upcard-1, random_action] += 1
                else:
                    no_usable_ace_returns[player_sum-12, dealer_upcard-1, random_action] = gamma*no_usable_ace_returns[player_sum-12, dealer_upcard-1, random_action] + reward
                    no_usable_ace_N[player_sum-12, dealer_upcard-1, random_action] += 1
